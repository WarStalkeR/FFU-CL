...
public class patch_PlayerAction_Plant : PlayerAction_Plant {
	public bool DetermineActive() {
		...
		if (!FFU_Connection_Lost.Data.isSemiCreative || !controller.cmd.onPlanet || controller.cmd.raycast == null || controller.gameData.guideRunning) return false;
...
public class patch_CargoTraffic : CargoTraffic {
	public void MonitorGameTick() {
		...
		bool sandboxToolsEnabled = FFU_Connection_Lost.Data.isSemiCreative;
		PerformanceMonitor.BeginSample(ECpuWorkEntry.Belt);
		for (int i = 1; i < monitorCursor; i++) if (monitorPool[i].id == i) monitorPool[i].InternalUpdate(this, sandboxToolsEnabled, entityPool, speakerPool, entityAnimPool);
...
public class patch_UIFunctionPanel : UIFunctionPanel {
	...
	protected override void _OnUpdate() {
		if (openedTime == 0f) menuTweener.normalizedTime = 0f;
		if (!FFU_Connection_Lost.Data.isSemiCreative) showSandBoxMenu = false;
		...
		gposWanted = (GameMain.data.guideRunning ? (-200f) : 0f);
		bgImage.color = color;
		...
		bool flag2 = !FFU_Connection_Lost.Data.isSemiCreative && !flag && !GameMain.mainPlayer.sailing && !showSandBoxMenu;
		...
		if (needResearchTipGroup.activeSelf != flag2) needResearchTipGroup.SetActive(flag2);
		bool flag3 = openedTime > 1f && guideComplete;
		UIGame uiGame = UIRoot.instance.uiGame;
		if (UIGame.viewMode == EViewMode.Starmap) flag3 = false;
		if (flag3 && menuTweener.normalizedTime < 1f) menuTweener.Play0To1Continuing();
		else if (!flag3 && menuTweener.normalizedTime > 0f) menuTweener.Play1To0Continuing();
		...
		sandboxButton.gameObject.SetActive(FFU_Connection_Lost.Data.isSemiCreative);
		sandboxButton.highlighted = showSandBoxMenu;
		PlanetData localPlanet = GameMain.localPlanet;
		bool fastTravelling = UIRoot.instance.uiGame.starmap.fastTravelling;
		bool flag4 = localPlanet != null && localPlanet.type == EPlanetType.Gas && mainPlayer.movementState <= EMovementState.Fly;
		...
		extractProgressImage.fillAmount = (fastTravelling ? 0f : playerController.actionMine.extractGasProgress[0]);
		if (VFInput._openBlueprintLibrary && VFInput.inScreen && !VFInput.inputing && !UIGridSplit.opened && buildMenu.blueprintButton.button.interactable) {
...
public class patch_UIMonitorWindow : UIMonitorWindow {
	...
	private void RefreshMonitorWindow() {
		...
		float num2 = cargoTraffic.monitorPool[monitorId].curPeriodTickCount / 60f;
		float num3 = num2 * 120f / (cargoFlowSlider.maxValue - cargoFlowSlider.minValue);
		float num4 = cargoTraffic.monitorPool[monitorId].targetCargoBytes / 10f;
		...
		if (FFU_Connection_Lost.Data.isSemiCreative) {
			...
			if (cargoTraffic.monitorPool[monitorId].spawnItemOperator > 0) {
				...
				spawnSwitch.SetImmediately(cargoTraffic.monitorPool[monitorId].spawnItemOperator == 1);
	...
	protected override void _OnOpen() {
		if (GameMain.localPlanet != null && GameMain.localPlanet.factory != null) {
			...
			TryOpenSpeakerPanel();
			UIItemPicker.showAll = FFU_Connection_Lost.Data.isSemiCreative;
		} else {
			_Close();
		}
...
public class patch_UIGame : UIGame {
	...
	protected override void _OnUpdate() {
		...
		if (functionPanel.showSandBoxMenu && FFU_Connection_Lost.Data.isSemiCreative) {
			buildMenu._Close();
		...
		warningWindow.Determine(!dysonEditor.active && !techTree.active);
		...
		if ((recipePicker.active || itemPicker.active || signalPicker.active || color256Picker.active) && VFInput.escape) {
			VFInput.UseEscape();
		...
		if (inventory.active && VFInput.escape) {
			VFInput.UseEscape();
			ShutPlayerInventory();
		}
		...
		if (guideComplete) {
			if (Input.GetKeyDown(KeyCode.E) && !VFInput.inputing && flag) {
				if (mechaWindow.active && colorPalettePanel.gameObject.activeInHierarchy) {
					...
					sandboxMenu.OnCategoryButtonClick(0);
				} else if (recipePicker.active || itemPicker.active || signalPicker.active || color256Picker.active) {
					if (recipePicker.active) {
						recipePicker._Close();
					}
					if (itemPicker.active) {
				...
				On_Y_Switch();
			} else if (VFInput._openMechLight && !VFInput.inputing) {
